...
void process_command(GCode *com,byte bufferedCommand)
{
  ...
  center of arc when h_x2_div_d is positive ->  x <----- | -----> x <- center of arc when h_x2_div_d is negative
    ...
    switch( com->M ) {
      ...
      case 115: {// M115
#if DRIVE_SYSTEM==3
        out.println_P(PSTR("FIRMWARE_NAME:Repetier_" REPETIER_VERSION ", PROTOCOL_VERSION:1.0*, MACHINE_TYPE:Delta (Kossel), EXTRUDER_COUNT:" XSTR(NUM_EXTRUDER) ", REPETIER_PROTOCOL:2"));
#else
#if DRIVE_SYSTEM==0
